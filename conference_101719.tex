...
\begin{table}[!t]
\caption{Workload axes used to interpret benchmark results.}
\label{tab:workload-axes}
\centering
\footnotesize
\begin{tabular}{@{}p{0.28\linewidth}p{0.58\linewidth}@{}}
\toprule
Axis & Examples and diagnostic purpose \\
\midrule
Query family & State check, as-of state, state diff, goal check \\
Predicate family & Unary state, relation, numeric state, contact, material state \\
Episode horizon & Short, medium, long episodes for scaling and drift analysis \\
Stream regime & Clean, missing, temporal disorder, conflict, mixed \\
Temporal position & Initial, transition-near, post-transition, final \\
Evidence status & Supported, missing, contradicted, uncertain, conflicting \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:perturbations} summarizes the regimes used in the benchmark design. The main release contains clean and hard mixed public streams. The broader protocol supports single-factor regimes for diagnosis, including missing observations, temporal disorder, low confidence, and explicit conflicts. The hard mixed stream is the main stress condition because embodied observation streams rarely contain a single isolated issue.

\begin{table}[!t]
\caption{Perturbation regimes and the data-management challenge they create.}
\label{tab:perturbations}
\centering
\footnotesize
\begin{tabular}{@{}p{0.24\linewidth}p{0.34\linewidth}p{0.29\linewidth}@{}}
\toprule
Regime & Stream issue & Maintenance challenge \\
\midrule
Clean & Simulator-derived observations & Basic state-view construction \\
Missing & Omitted evidence near transitions & Unknown and stale-state handling \\
Temporal disorder & Delayed or out-of-order arrival & Valid-time and transaction-time repair \\
Conflict & Support and contradict evidence coexist & Confidence and provenance tracking \\
Mixed & Multiple issues combined & Robustness under realistic stream defects \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[!t]
\caption{Primary systems and expected stress points.}
\label{tab:baselines}
\centering
\footnotesize
\begin{tabular}{@{}p{0.24\linewidth}p{0.31\linewidth}p{0.31\linewidth}@{}}
\toprule
System & Representation & Expected weakness \\
\midrule
Recall memory & Retrieved evidence snippets & State diffs and goal views \\
Temporal log & Append-only observations & Structured task-derived views \\
\engine & Bitemporal state view & Update overhead and design complexity \\
\bottomrule
\end{tabular}
\end{table}
...
The first lesson is that latest-state memory is not a sufficient abstraction for embodied task state. Late evidence can revise the answer to an as-of query, and contradictory evidence can make the correct answer uncertain or conflicting rather than simply true or false.

The second lesson is that retrieval and view maintenance answer different questions. Retrieval can surface relevant evidence, but a task executor often needs a maintained predicate value, a change set, or a goal-satisfaction view. \bench\ therefore evaluates QueryAnswers, not retrieved frames alone.

The third lesson is that provenance should be part of the benchmark design even when the first main release emphasizes core state answers. Support and contradict evidence are needed for why queries, failure localization, and trust calibration. They also help diagnose whether a system was correct for the right reason.

The fourth lesson is that goal views are more than a convenience layer. A system may answer individual predicate checks correctly but still fail to aggregate them into a task-level answer if it mishandles unknown or conflicting predicates. Conversely, a goal-level score can hide which predicate failed. \bench\ therefore reports both goal satisfaction and goal-predicate F1.

The fifth lesson is that benchmark artifacts need validation reports as first-class objects. Without public validation, a user cannot tell whether a failure comes from their system, from malformed query metadata, from leaked answer fields, or from inconsistent task specifications. The reported release treats validation status, stream counts, and query counts as part of the benchmark profile.

\subsection{Limitations}

\bench\ makes a controlled isolation choice. The main artifact is simulator-grounded and structured; it does not claim to solve raw perception or low-level robot control. This is a limitation, but also a design requirement for a reproducible data-management benchmark. If perception and control are folded into the main score too early, failures become hard to attribute.

The reported public release focuses on structured simulator-grounded observations. It includes provenance, precondition, uncertain-state, and failure-localization queries, but raw perception-derived streams are not used as the main correctness source. This keeps the evaluation reproducible, but it limits claims about real sensor distributions.

Finally, the reported baseline set is intentionally conservative. It emphasizes transparent systems whose failure modes can be analyzed under the benchmark. Neural memory, LLM-based agents, and full embodied pipelines are outside this baseline set; they fit the benchmark only when their public inputs, inference budgets, and output schemas can be made comparable.

\subsection{Artifact Availability}

The benchmark release provides public task specifications, public observation streams, query sets, manifests, validation reports, build commands, baseline implementations, prediction schemas, and evaluator scripts. Hidden timelines and answer sets are used for evaluation. Artifact hosting must avoid reviewer tracking and follow the ICDE submission instructions.

The release provides a minimal reproduction path: build or validate public artifacts, run each baseline on the hard mixed stream, validate prediction schema, evaluate predictions, and regenerate the reported tables and figures. For EAB review, this path is as important as the dataset itself because it shows that the benchmark is executable rather than only descriptive.
...
ALFRED evaluates instruction following for household tasks~\cite{shridhar2020alfred}. OpenEQA evaluates embodied question answering over episodic memory and active exploration~\cite{majumdar2024openeqa}. These benchmarks ask whether an agent can act or answer language questions. \bench\ asks whether a system can maintain and query a temporal state view from imperfect evidence.

Work on embodied memories and scene representations is complementary. Such systems often store observations, scene graphs, maps, or retrieved context for downstream decisions. \bench\ can evaluate those systems if they can produce QueryAnswers from public streams, but it does not require a particular memory architecture. This representation-agnostic design is important because a benchmark should compare methods without prescribing their internal storage model.

\subsection{Temporal, Stream, and Provenance Data Management}

Temporal databases separate the time when a fact is valid in the modeled world from the time when a database records or knows the fact~\cite{snodgrass1999developing,jensen1999temporal}. Stream systems study continuous processing over arriving data~\cite{babcock2002models,garofalakis2016data}. Incremental view maintenance and materialized-view maintenance study how derived database state can be updated after base changes~\cite{gupta1995maintenance,zhuge1995view}. Provenance models record how query results are derived from input data~\cite{green2007provenance,cheney2009provenance}. Uncertain and probabilistic data management studies query answering when input facts are noisy or probabilistic~\cite{aggarwal2009uncertain,dalvi2007efficient}. \bench\ brings these ideas into embodied task-state maintenance, where event-time and arrival-time mismatch, missing observations, conflicting evidence, and task-derived goal views appear together.

The benchmark also connects to incremental view maintenance and uncertain data. The maintained \tsv\ can be seen as a materialized view over an observation log, but its updates include late-arrival repair, contradict evidence, confidence changes, and task-derived aggregation. \bench\ does not propose a single optimal view-maintenance algorithm. Instead, it defines the workload that competing algorithms must satisfy.

\subsection{Experiment, Analysis, and Benchmark Papers}

Database benchmark papers often contribute workload models, artifact rules, and diagnostic analyses rather than only a new algorithm. LDBC benchmarks standardize graph workloads and reporting rules for social-network and graph-analytics settings~\cite{erling2015ldbc,iosup2016ldbc}; TSM-Bench benchmarks time-series database systems for monitoring applications~\cite{khelifati2023tsmbench}; and FinBench defines a financial graph transaction workload~\cite{qi2025finbench}. Recent EAB papers similarly contribute unified evaluation frameworks and analysis of method trade-offs: SQLMorph mutates text-to-SQL workloads and introduces fine-grained metrics~\cite{malekpour2026sqlmorph}; BEACON provides a benchmark framework for subgraph counting~\cite{najafi2025beacon}; and plan-based AQP analysis decomposes system mechanisms to explain performance differences~\cite{mu2025aqp}. \bench\ follows this EAB pattern: the contribution is the benchmark artifact, workload, evaluator, and analysis of where baseline strategies succeed or fail.

This EAB framing also guides the paper structure. The paper spends substantial space on artifact design, query semantics, metrics, validation, and interpretation of failures. A thin benchmark description followed by a large system evaluation would misrepresent the contribution; the benchmark itself is the main object.
